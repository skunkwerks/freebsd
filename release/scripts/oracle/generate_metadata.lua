...
if not os_type or not os_version or not capability_file or
   not shapes_file or not template_file or not output_file then
    io.stderr:write("Error: Oracle script is missing required environment variables:\n")
...
local function read_file(path)
    ...
    if not f then
        io.stderr:write("Error: Oracle script cannot open file: " .. path .. "\n")
        os.exit(1)
    end
...
if not caps_parser:parse_string(caps) then
    io.stderr:write("Error: Oracle script found invalid JSON in capability file\n")
    os.exit(1)
end
...
if not out then
    io.stderr:write("Error: Oracle script cannot create output file: " .. output_file .. "\n")
    os.exit(1)
end
